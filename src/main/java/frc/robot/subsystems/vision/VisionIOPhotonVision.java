...
public class VisionIOPhotonVision implements VisionIO {
	...
	// Put these values into an array to iterate through them (go through them in
	// order)
	...
	public VisionIOPhotonVision() {
		...
		// Set the strategy to use when multitag isn't detected. IE, only one apriltag
		// visible
		...
		if (Constants.currentMode == Mode.SIM) {
			...
			/*
			 * We use ARDUCAM OV9281s, which are the same cameras as on the limelight 3G.
			 * BLACK AND WHITE
			...
			for (var i = 0; i < cams.length; i++) {
				...
				// the following are cool to see in sim, but take over 80ms to run, so use at
				// risk for debugging
	...
	public double[] getEstimationFactors(Pose2d estimatedPose,
			PhotonPoseEstimator photonEstimator, PhotonPipelineResult result,
	...
	public void updateInputs(VisionIOInputs inputs) {
		...
		if (Constants.currentMode == Mode.SIM) {
			visionSim.update(RobotContainer.fieldSimulation
					.getMainDriveSimulation().getPose3d().toPose2d());
		}
		...
		for (int i = 0; i < cams.length; i++) {
			...
			if (cCam.isConnected()) {
				// get list of unread results
				List<PhotonPipelineResult> unreadResults = cCam.getAllUnreadResults();
				for (PhotonPipelineResult result : unreadResults) {
					var visionEst = getEstimatedGlobalPose(cEstimator, cCam, result,
					...
					visionEst.ifPresentOrElse(est -> {
						...
						/*
						 * for (PhotonTrackedTarget target : est.targetsUsed) { //To test the Offset
						 * feature, uncomment this.
	 ...
	 */
	public static Optional<Pose2d> aprilTagPose2d(PhotonPipelineResult result,
			int tagToLookFor) {
		var targets = result.getTargets();
	...
	public static boolean aprilTagVisible(PhotonCamera camera) {
		...
		return !camera.getAllUnreadResults().get(0).getTargets().isEmpty();
	}
	...
	public Optional<EstimatedRobotPose> getEstimatedGlobalPose(
			PhotonPoseEstimator photonEstimator, PhotonCamera camera, PhotonPipelineResult result,
