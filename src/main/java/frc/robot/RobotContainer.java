...
import frc.robot.subsystems.simpleMechanisms.roller.shooter.ShooterIOSim;
...
public class RobotContainer {
	...
	// static Trigger BranchOneScoreTrigger = new Trigger(() -> dsHIDHandler.getBranch1Button()); create controller buttons for each needed. map out controller functions.
	...
	public static double[] getCurrentDraw() {
		...
		return new double[] { Math.min(drivetrainS.getCurrent(), 200), intake.getCurrent() };
	...
	public static HashMap<String, Double> getAllTemps() {
		// List of HashMaps
		List<HashMap<String, Double>> maps = List.of(drivetrainS.getTemps(), intake.getTemps());
