...
import frc.robot.utils.CompetitionFieldUtils.Simulation.motorsims.*;
 ...
 * <p>
 * Check <a href=
 * 'https://shenzhen-robotics-alliance.github.io/maple-sim/swerve-sim-hardware-abstraction/'>Online
 * Documentation</a>
 *
 * <p>
 * This class provides a simulation for a single swerve module in the
 ...
 *
 * <p>
 * You will apply voltage outputs to the drive/steer motor of the module and
 * obtain their encoder readings in your
 * code, just as how you deal with your physical motors.
 *
 * <h3>2. Perspectives</h3>
 *
 * <ul>
 * <li>Simulates the steering mechanism using a custom brushless motor
 * simulator.
 * <li>Simulates the propelling force generated by the driving motor, with a
 * current limit.
 * <li>Simulates encoder readings, which can be used to simulate a
 ...
 *
 * <ul>
 * <li>Retrieve the encoder readings from
 * {@link #getDriveEncoderUnGearedPosition()}} and
 ...
 * Odometry</a> is supported. You can retrive cached encoder readings from every
 * sub-tick through
 * {@link #getCachedDriveEncoderUnGearedPositions()} and
 * {@link #getCachedSteerAbsolutePositions()}.
 * </ul>
 *
 * <p>
 * An example of how to simulate odometry using this class is the <a
 * href=
...
public class SwerveModuleSimulation {
     ...
     * to use custom timings, you must
     * call the method before constructing any swerve module simulations using this
     * constructor.
     *
     * @param driveMotorModel           the model of the driving motor
     * @param steerMotorModel;          the model of the steering motor
     * @param driveGearRatio            the gear ratio for the driving motor, >1 is
     *                                  reduction
     * @param steerGearRatio            the gear ratio for the steering motor, >1 is
     *                                  reduction
     * @param driveFrictionVoltage      the measured minimum amount of voltage that
     *                                  can turn the driving rotter
     * @param steerFrictionVoltage      the measured minimum amount of voltage that
     *                                  can turn the steering rotter
     * @param wheelRadius               the radius of the wheels.
     * @param steerRotationalInertia    the rotational inertia of the entire
     *                                  steering mechanism
     * @param tireCoefficientOfFriction the <a
     *                                  href=
     *                                  'https://simple.wikipedia.org/wiki/Coefficient_of_friction#:~:text=A%20coefficient%20of%20friction%20is%20a%20value%20that%20shows%20the'>coefficient
     *                                  of friction</a> of the tires, normally
     *                                  around 1.2
     ...
     *
     * <p>
     * Think of it as the <code>requestOutput()</code> of your physical steering
     * motor.
     *
     ...
     *
     * <p>
     * This value represents the un-geared position of the encoder, i.e., the amount
     * of radians the drive motor's
     * encoder has rotated.
     *
     ...
     *
     * <p>
     * This method provides the final position of the drive encoder in terms of
     * wheel angle.
     *
     ...
     *
     *
     * <h2>Obtains the Cached Readings of the Drive Encoder's Un-Geared
     * Position.</h2>
     *
     * <p>
     * The values of {@link #getDriveEncoderUnGearedPosition()} are cached at each
     * sub-tick and can be retrieved
     * using this method.
     *
     ...
     *
     *
     * <h2>Obtains the Cached Readings of the Drive Encoder's Final Position (Wheel
     * Rotations).</h2>
     *
     * <p>
     * The values of {@link #getDriveWheelFinalPosition()} are cached at each
     * sub-tick and are divided by the gear
     ...
     *
     *
     * <h2>Obtains the Cached Readings of the Steer Relative Encoder's
     * Position.</h2>
     *
     * <p>
     * The values of {@link #getSteerRelativeEncoderPosition()} are cached at each
     * sub-tick and can be retrieved
     * using this method.
     *
    ...
    public Angle[] getCachedSteerRelativeEncoderPositions() {
        return steerAbsolutePositionCache.stream()
                .map(absoluteFacing -> absoluteFacing.getMeasure().times(STEER_GEAR_RATIO)
     ...
     *
     * <p>
     * The values of {@link #getSteerAbsoluteFacing()} are cached at each sub-tick
     * and can be retrieved using this
     * method.
     *
     * @return an array of cached absolute steer positions, as {@link Rotation2d}
     *         objects
     */
     ...
     *
     * <p>
     * <strong>Note:</strong> Friction forces are not simulated in this method.
     *
     * @param moduleCurrentGroundVelocityWorldRelative the current ground velocity
     *                                                 of the module, relative to
     *                                                 the world
     * @param robotFacing                              the absolute facing of the
     *                                                 robot, relative to the world
     * @param gravityForceOnModuleNewtons              the gravitational force
     *                                                 acting on this module, in
     *                                                 newtons
     * @return the propelling force generated by the module, as a {@link Vector2}
     *         object
     */
    public Vector2 updateSimulationSubTickGetModuleForce(
        ...
        final Vector2 propellingForce = getPropellingForce(grippingForceNewtons, moduleWorldFacing,
    ...
    private void updateSteerSimulation() {
        ...
        steerMotorSim.update(Seconds.of(DriveConstants.RobotPhysicsSimulationConfigs.SIMULATION_DT));
     ...
     *
     * <p>
     * For most of the time, that propelling force is directly applied to the
     * drivetrain. And the drive wheel runs as
     * fast as the ground velocity
     *
     * <p>
     * However, if the propelling force exceeds the gripping, only the max gripping
     * force is applied. The rest of the
     * propelling force will cause the wheel to start skidding and make the odometry
     * inaccurate.
     *
     * @param grippingForceNewtons        the amount of gripping force that wheel
     *                                    can generate, in newtons
     * @param moduleWorldFacing           the current world facing of the module
     * @param moduleCurrentGroundVelocity the current ground velocity of the module,
     *                                    world-reference
     * @return a vector representing the propelling force that the module generates,
     *         world-reference
     */
    private Vector2 getPropellingForce(
        ...
        // to the wheel
        this.driveWheelFinalSpeed = RadiansPerSecond
                .of(floorVelocityProjectionOnWheelDirectionMPS / WHEEL_RADIUS.in(Meters));
     ...
     *
     *
     * <h2>Calculates the amount of torque that the drive motor can generate on the
     * wheel.</h2>
     *
     * @return the amount of torque on the wheel by the drive motor, in Newton *
     *         Meters
     */
     ...
     *
     * <p>
     * The "free spin" state of a simulated module refers to its state after
     * spinning freely for a long time under
    ...
    private void updateEncoderCaches() {
        ...
        this.driveWheelFinalPosition = this.driveWheelFinalPosition.plus(this.driveWheelFinalSpeed
                .times(Seconds.of(DriveConstants.RobotPhysicsSimulationConfigs.SIMULATION_DT)));
     ...
     *
     * @return the theoretical maximum ground speed that the module can achieve, in
     *         m/s
     */
     ...
     *
     * <p>
     * Calculates the maximum propelling force with respect to the gripping force
     * and the drive motor's torque under
     * its current limit.
     *
     * @param robotMass    the mass of the robot
     * @param modulesCount the amount of modules on the robot, assumed to be sharing
     *                     the gravity force equally
    ...
    public Force getTheoreticalPropellingForcePerModule(Mass robotMass, int modulesCount) {
        final double maxThrustNewtons = driveMotorConfigs
     ...
     *
     *
     * <h2>Obtains the theatrical linear acceleration that the robot can
     * achieve.</h2>
     *
     * <p>
