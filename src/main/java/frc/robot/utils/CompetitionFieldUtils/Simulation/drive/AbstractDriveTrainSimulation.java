 ...
 * <p>
 * This class models the physical properties of a drivetrain, including mass,
 * collision space, and friction.
 * <p>
 * The propelling forces generated by motors are simulated in its subclasses,
 * such as {@link SimplifiedHolonomicDriveSimulation} or
...
public abstract class AbstractDriveTrainSimulation extends Body implements RobotOnFieldDisplay{
	...
	public final Consumer<Pose2d> resetOdometryCallBack;
	/**
	 * <h2>Creates a Simulation of a Drivetrain.</h2>
	 * <h3>Sets Up the Collision Space and Mass of the Chassis.</h3>
	 * <p>
	 * Since this is an abstract class, the constructor must be called from a
	 * subclass.
	 * <p>
	 * Note that the chassis does not appear on the simulation field upon
	 * creation. Refer to {@link org.ironmaple.simulation.SimulatedArena} for
	 * instructions on how to add it to the simulation world.
	 *
	 * @param profile            a profile class that stores all the
	 *                              configurations for this drivetrain, see
	 *                              {@link DriveTrainSimulationProfile}
	 * @param initialPoseOnField the initial pose of the drivetrain in the
	 *                              simulation world
	 */
	protected AbstractDriveTrainSimulation(DriveTrainSimulationProfile profile,
		...
		super.addFixture(
				...
				BUMPER_COEFFICIENT_OF_FRICTION, BUMPER_COEFFICIENT_OF_RESTITUTION);
	 ...
	 * <p>
	 * This method instantly teleports the robot to the specified pose in the
	 * simulation world. The robot does not drive to the new pose; it is moved
	 * directly.
	 *
	...
	public void resetOdometryToActualRobotPose(){
		resetOdometryCallBack.accept(getSimulatedDriveTrainPose());
	}
	/**
	 * <h2>Sets the Robot's Speeds to the Given Chassis Speeds.</h2>
	 * <p>
	 * This method sets the robot's current velocity to the specified chassis
	 * speeds.
	 * <p>
	 * The robot does not accelerate smoothly to these speeds; instead, it jumps
	 ...
	 * <p>
	 * It is implemented in the sub-classes of
	 * {@link AbstractDriveTrainSimulation}.
	 * <p>
	 * It is responsible for applying the propelling forces to the robot during
	 * each sub-tick of the simulation.
	 */
	 ...
	 * <p>
	 * This method simulates the linear friction forces on the drivetrain and
	 * applies them to the physics engine.
	 * <p>
	 * It should be called from the implementation of
	 * {@link #simulationSubTick()}, particularly when the drivetrain is not
	 * attempting to move linearly.
	 */
	protected void simulateChassisLinearFriction() {
		final double actualLinearVelocityPercent = getLinearVelocity()
				.getMagnitude() / profile.maxLinearVelocity;
	 ...
	 * <p>
	 * This method simulates the angular friction forces on the drivetrain and
	 * applies them to the physics engine.
	 * <p>
	 * It should be called from the implementation of
	 * {@link #simulationSubTick()}, particularly when the drivetrain is not
	 * attempting to move rotationally.
	 */
	protected void simulateChassisAngularFriction(
		...
		final double actualRotationalMotionPercent = Math
				...
				differenceBetweenFloorSpeedAndModuleSpeed = desiredRotationalMotionPercent
						* profile.maxAngularVelocity - getAngularVelocity(),
				FRICTION_TORQUE_GAIN = 1;
		if (actualRotationalMotionPercent < 0.01
				&& Math.abs(desiredRotationalMotionPercent) < 0.02)
	 ...
	 * <p>
	 * This method is used to display the robot on <a href=
	 * "https://docs.advantagescope.org/tab-reference/3d-field/">AdvantageScope
	 * Field3d</a> or to <a href=
	 * "https://docs.photonvision.org/en/latest/docs/simulation/simulation-java.html#updating-the-simulation-world">update
	...
	public Pose3d getPose3d() {
		return new Pose3d(getSimulatedDriveTrainPose());
	}
	/**
	 * <h2>Gets the Actual Robot-Relative Chassis Speeds from the
	 * Simulation.</h2>
	 * <p>
	 * This method returns the actual chassis speeds of the drivetrain in the
	 * simulation, relative to the robot.
	 * <p>
	...
	/**
	 * <h2>Gets the Actual Field-Relative Chassis Speeds from the
	 * Simulation.</h2>
	 * <p>
	 * This method returns the actual chassis speeds of the drivetrain in the
	 * simulation, relative to the robot.
	...
	public static final class DriveTrainSimulationProfile {
		public final double maxLinearVelocity, maxLinearAcceleration,
				maxAngularVelocity, maxAngularAcceleration, robotMass, width,
				length;
		private double frictionForceMagnitudeNewtons, linearVelocityDamping,
				angularFrictionTorqueMagnitude, angularVelocityDamping;

		public DriveTrainSimulationProfile(double maxLinearVelocity,
				double maxLinearAcceleration, double maxAngularVelocity,
				double maxAngularAcceleration, double robotMass, double width,
				double length) {
			...
			this.linearVelocityDamping = maxLinearAcceleration / maxLinearVelocity
					* 0.75;
			this.angularFrictionTorqueMagnitude = frictionForceMagnitudeNewtons
					* DRIVE_BASE_RADIUS / 2;
			this.angularVelocityDamping = maxAngularAcceleration
					/ maxAngularVelocity * 0.75;
		}
		...
		public DriveTrainSimulationProfile withLinearVelocityDamping(
				double linearVelocityDamping) {
		...
		public DriveTrainSimulationProfile withAngularVelocityDamping(
				double angularVelocityDamping) {
		...
		public String toString() {
			return String.format(
					"RobotProfile { robotMaxVelocity=%.2f(m/s), robotMaxAcceleration=%.2f(m/s^2), robotMass=%.2f(kg), "
							...
							+ "length=%.2f(m) }",
					maxLinearVelocity, maxLinearAcceleration, robotMass,
					frictionForceMagnitudeNewtons, linearVelocityDamping,
					maxAngularVelocity, maxAngularAcceleration,
					angularVelocityDamping, angularFrictionTorqueMagnitude, width,
					length);
		}
	}
	...
	public class OdometryTimeStampsSim {
		public static double[] getTimeStamps() {
			final double[] odometryTimestamps = new double[RobotPhysicsSimulationConfigs.SIM_ITERATIONS_PER_ROBOT_PERIOD];
			for (int i = 0; i < RobotPhysicsSimulationConfigs.SIM_ITERATIONS_PER_ROBOT_PERIOD; i++)
				odometryTimestamps[i] = TimeUtil.getRealTimeSeconds()
						- Robot.defaultPeriodSecs
						+ RobotPhysicsSimulationConfigs.SIMULATION_DT * i;
		...
		public static final class OdometryThreadSim implements OdometryThread {
			...
			public void updateInputs(OdometryThreadInputs inputs) {
				inputs.measurementTimeStamps = OdometryTimeStampsSim
						.getTimeStamps();
			}
		}
	}
