...
public class GyroSimulation {
   ...
   *
   * @param AVERAGE_DRIFTING_IN_30_SECS_MOTIONLESS_DEG the average amount of drift, in degrees, the
   *     gyro experiences if it remains motionless for 30 seconds on a vibrating platform. This
   *     value can often be found in the user manual.
  ...
  public GyroSimulation(
    ...
    for (int i = 0; i < RobotPhysicsSimulationConfigs.SIM_ITERATIONS_PER_ROBOT_PERIOD; i++)
   ...
   * simulation.
   *
   * <p>If you are using this class outside of {@link org.ironmaple.simulation.SimulatedArena}: make
  ...
  public void updateSimulationSubTick(double actualAngularVelocityRadPerSec) {
	 Logger.recordOutput("Drive/Gyro/angular velocity (Deg per Sec)", Math.toDegrees(actualAngularVelocityRadPerSec));
    ...
    final Rotation2d dTheta = getGyroDTheta(actualAngularVelocityRadPerSec);
	 Logger.recordOutput("Drive/Gyro/dtheta", dTheta.getDegrees());
    ...
    final double
        angularAccelerationRadPerSecSq =
            (actualAngularVelocityRadPerSec - previousAngularVelocityRadPerSec)
                / RobotPhysicsSimulationConfigs.SIMULATION_DT,
  ...
  private Rotation2d getGyroDTheta(double actualAngularVelocityRadPerSec) {
    this.measuredAngularVelocityRadPerSec =
        CommonMath.generateRandomNormal(
            actualAngularVelocityRadPerSec,
            VELOCITY_MEASUREMENT_STANDARD_DEVIATION_PERCENT
                * Math.abs(actualAngularVelocityRadPerSec));
    return Rotation2d.fromRadians(
        measuredAngularVelocityRadPerSec * RobotPhysicsSimulationConfigs.SIMULATION_DT);
  }
  ...
  private Rotation2d getNoMotionDrifting() {
    final double
        AVERAGE_DRIFTING_1_PERIOD =
            this.AVERAGE_DRIFTING_IN_30_SECS_MOTIONLESS_DEG / 30 * RobotPhysicsSimulationConfigs.SIMULATION_DT,
        driftingInThisPeriod = CommonMath.generateRandomNormal(0, AVERAGE_DRIFTING_1_PERIOD);
